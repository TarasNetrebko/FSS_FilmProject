...
const firebaseConfig = {
  ...
  databaseURL:
    'https://auth-app-16518-default-rtdb.europe-west1.firebasedatabase.app',
...
export default function createModal(data) {
  ...
  function addFilmToWatched() {
    set(
      ref(database, `users/${userId}/watchedMovies/${id}`),
  ...
  function addFilmToQueue() {
    set(
      ref(database, `users/${userId}/queueOfMovies/${id}`),
      JSON.stringify(data.data)
    );
    const dbRef = ref(getDatabase());
    get(child(dbRef, `users/${userId}/queueOfMovies`))
...
`,
    {
      onShow: instance => {
        document.querySelector('body').style.overflow = 'hidden';
        instance.element().querySelector('.modal-button-x').onclick =
          instance.close;
      },
      onClose: instance => {
        document.querySelector('body').style.overflow = 'auto';
